...
function [u resid] = sim1D(d1,alphadot,offset,dt,A,p,f,ds,fov,npixels,f0,noisesigma,derivsigma,trueu)
...
%     figure; hold on; plot(I0); plot(I1,'g'); plot(I2,'r'); legend('I0','I1','I2');
     ...
     % derivsigma = noisesigma; % may change this later
     if derivsigma
        derivgauss = normr(exp(-(-6*derivsigma:6*derivsigma).^2/(2*derivsigma^2)));
     else
         derivgauss = 1;
     end
     deriv1 = conv(derivgauss,[-.5, 0, .5]);
     Ix = conv(I1,deriv1,'same');
     xIx = X.*Ix;
     Ixx = conv(Ix,deriv1,'same');
     It = (I2-I0)/(2*dt); % need whole movie to do smooth derivative
     
     % trim edges
     trim = @(v) v(1+ceil(2*derivsigma):end-ceil(2*derivsigma));
    ...
    if A > 0
        ...
        % calculate residual
        resid = norm(spacederivs*trueu-timederivs);
...
%        ttc = 1/u(2);
%        dir = u(1)/u(2);
        resid = norm(spacederivs*trueu(1:2)-timederivs);
    end
